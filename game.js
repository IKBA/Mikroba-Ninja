...
class GameArea {
    ...
    triggerLevelUp(level) {
        ...
        // SFX: Level Up (hanya dipanggil sekali dari sisi kiri)
        if (this.side === 'left') {
    ...
    handleMicrobeSliced(microbe, index, angle) {
        ...
        if (microbe.type === this.targetType) {
            ...
            // SFX: Tebasan benar
            audioEngine.playSliceCorrect(this.side);
            ...
            // SFX: Tebasan salah / bom
            audioEngine.playSliceBomb();
...
function initGame() {
    ...
    // Setup tombol audio (mute/volume controls)
    ...
    // Hover effect untuk tombol audio
    audioToggleBtn.addEventListener('mouseenter', () => {
        if (audioEngine.isInitialized) {
            audioEngine.playButtonHover();
        }
    });

    audioToggleBtn.addEventListener('click', (e) => {
        e.stopPropagation(); // Mencegah terpicunya klik di luar panel
        audioEngine.init(); // Pastikan audio context aktif
        audioEngine.resume();
    ...
    // Menghubungkan slider volume musik
    sliderMusic.addEventListener('input', (e) => {
        const val = parseInt(e.target.value) / 100;
        audioEngine.setMusicVolume(val);
        musicVolValue.innerText = Math.round(val * 100) + '%';
        updateAudioIcons();
    });

    // Menghubungkan slider volume SFX
    sliderSfx.addEventListener('input', (e) => {
        const val = parseInt(e.target.value) / 100;
        audioEngine.setSfxVolume(val);
        sfxVolValue.innerText = Math.round(val * 100) + '%';
        updateAudioIcons();
    });

    // Mainkan suara chime pendek saat user selesai menggeser slider SFX (feedback volume)
    sliderSfx.addEventListener('change', () => {
        audioEngine.playButtonClick();
    });

    // Fungsi pembantu untuk memperbarui ikon speaker utama jika semua volume 0 (mute visual)
    function updateAudioIcons() {
        const totalVol = parseFloat(sliderMusic.value) + parseFloat(sliderSfx.value);
        if (totalVol === 0) {
    ...
    // Klik di luar panel audio untuk menutup panel secara otomatis
    document.addEventListener('click', (e) => {
        if (audioPanel.classList.contains('active') && !audioPanel.contains(e.target) && !audioToggleBtn.contains(e.target)) {
...
function startGame() {
    ...
    // Inisialisasi & mulai audio (harus dari event user click)
    audioEngine.init();
    audioEngine.resume();
    audioEngine.playGameStart();
    
    // Mulai background music setelah SFX boot-up selesai
    setTimeout(() => {
...
function endGame() {
    ...
    // Audio: hentikan musik & mainkan SFX game over
